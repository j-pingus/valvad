import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { PermitService } from '../service/permit.service';

import { PermitComponent } from './permit.component';

describe('Permit Management Component', () => {
  let comp: PermitComponent;
  let fixture: ComponentFixture<PermitComponent>;
  let service: PermitService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'permit', component: PermitComponent }]), HttpClientTestingModule],
      declarations: [PermitComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: { snapshot: { queryParams: {} } },
        },
      ],
    })
      .overrideTemplate(PermitComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(PermitComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(PermitService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.permits?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
