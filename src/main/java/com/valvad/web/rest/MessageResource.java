...
/**
 * REST controller for managing {@link com.valvad.domain.Message}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MessageResource {
        ...
        Optional<Message> result = messageRepository
            ...
            .map(existingMessage -> {
                ...
                if (message.getSubjectId() != null) {
                    existingMessage.setSubjectId(message.getSubjectId());
                }
    ...
    public List<Message> searchMessages(@RequestParam String query) {
        ...
        return StreamSupport.stream(messageSearchRepository.search(query).spliterator(), false).collect(Collectors.toList());
    }
}
