...
/**
 * REST controller for managing {@link com.valvad.domain.Model}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ModelResource {
        ...
        Optional<Model> result = modelRepository
            ...
            .map(existingModel -> {
                ...
                if (model.getYears() != null) {
                    existingModel.setYears(model.getYears());
                }
            ...
            .map(savedModel -> {
                modelSearchRepository.save(savedModel);
    ...
    public List<Model> searchModels(@RequestParam String query) {
        ...
        return StreamSupport.stream(modelSearchRepository.search(query).spliterator(), false).collect(Collectors.toList());
    }
}
